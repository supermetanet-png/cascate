...
const RLSManager: React.FC<{ projectId: string }> = ({ projectId }) => {
    ...
    try {
      ...
      if (curr?.security_config) {
         ...
         setSecurityConfig({
           ...
           table_permissions: cfg.table_permissions || {}
         });
      }
  ...
  const saveGovernor = async () => {
    ...
    try {
      ...
      setSuccessMsg('Cascata Governor Security Deployed.');
  ...
  const updateTablePerm = (table: string, role: string, op: string, val: boolean) => {
    setSecurityConfig((prev: any) => {
      const next = { ...prev };
      const tablePerms = { ...(next.table_permissions || {}) };
      const tableData = { ...(tablePerms[table] || {}) };
      const roleData = { ...(tableData[role] || { create: true, read: true, update: true, delete: true }) };
      
      roleData[op] = val;
      tableData[role] = roleData;
      tablePerms[table] = tableData;
      next.table_permissions = tablePerms;
    ...
    <div className="flex flex-col h-full bg-[#FAFBFC] overflow-hidden">
      ...
      <header className="px-10 py-8 bg-white border-b border-slate-200 flex items-center justify-between shadow-sm z-10">
        <div className="flex items-center gap-4">
          ...
          <div><h1 className="text-2xl font-black text-slate-900 tracking-tighter leading-none">Security Center</h1><p className="text-[10px] text-indigo-600 font-black uppercase tracking-widest mt-1">Multi-Layer Access Control</p></div>
        </div>
        ...
        {activeTab === 'policies' ? (
          <div className="max-w-6xl mx-auto space-y-8 pb-40">
             <div className="bg-indigo-600 rounded-[2.5rem] p-10 text-white flex items-center gap-8 shadow-2xl relative overflow-hidden mb-12">
                ...
                <div><h3 className="text-2xl font-black mb-1 italic">Physical Layer (RLS)</h3><p className="text-indigo-100 text-sm font-medium opacity-80 max-w-2xl">Políticas de Segurança de Nível de Linha processadas diretamente no kernel do banco de dados.</p></div>
             </div>
          ...
          <div className="max-w-6xl mx-auto space-y-12 pb-40">
             <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white flex items-center justify-between shadow-2xl relative overflow-hidden">
                <div className="flex items-center gap-8 relative z-10">
                   ...
                   <div><h3 className="text-2xl font-black mb-1 italic">Cascata Governor</h3><p className="text-slate-400 text-sm font-medium max-w-xl">Lógica de segurança de aplicação que intercepta requisições antes do processamento.</p></div>
                </div>
                <button onClick={saveGovernor} disabled={executing} className="bg-white text-slate-900 px-8 py-4 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center gap-3 hover:bg-indigo-50 transition-all shadow-xl">
                  {executing ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} Update Governor
                </button>
             </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div className="bg-white border border-slate-200 rounded-[3rem] p-10 shadow-sm space-y-6">
                   ...
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Global Requests/min</p>
                ...
                <div className="lg:col-span-2 bg-white border border-slate-200 rounded-[3rem] p-10 shadow-sm space-y-8">
                   <h4 className="font-black text-slate-900 uppercase text-xs tracking-widest flex items-center gap-3"><Database size={18} className="text-emerald-600"/> Logical Permissions</h4>
...
const PolicyCard = ({ policy }: any) => (
  <div className="bg-slate-50 border border-slate-100 p-6 rounded-2xl flex items-center justify-between hover:border-indigo-200 transition-all">
    ...
    <div className="text-right">
       <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">SQL Qual</span>
